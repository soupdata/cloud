 ...
 * View 基类<br>
 *
 * @describe：封装 JPA CriteriaBuilder 查询条件
 */
@Slf4j
...
public class Query implements Serializable {
  ...
  private void addSubQuery(String propertyName, Query query) {
    ...
    if (query.projection == null)
      throw new RuntimeException("Subselect's fields must not be empty.");
...
// }
  ...
  /**
   * 直接添加 JPA 内部的查询条件,用于应付一些复杂查询的情况,例如或
   */
  ...
  public void addOrder(String propertyName, String order) {
    ...
    if (order.equalsIgnoreCase("asc"))
      this.orderBy.add(criteriaBuilder.asc(from.get(propertyName)));
    else if (order.equalsIgnoreCase("desc"))
      this.orderBy.add(criteriaBuilder.desc(from.get(propertyName)));
  }
  ...
  public void setOrders(List<Order> orderBy) {
